...
export async function extractAndStoreMemory({relationshipId, userMessageId, assistantMessageId, llm, embeddingProvider, signal}) {
    const [relationship, user, assistant] = await Promise.all([
        ...
        MessageModel.findOne({_id: userMessageId, relationshipId, role: "user", status: "completed"}).lean(),
        MessageModel.findOne({_id: assistantMessageId, relationshipId, role: "assistant", status: "completed", replyToMessageId: userMessageId}).lean(),
    ]);
    if (!relationship || !user || !assistant) throw new Error("Invalid memory extraction source");
    if (assistant.memoryExtractedAt || assistant.sequenceNumber <= (relationship.contextAfterSequence || 0)) return;
    const recent = await MessageModel.find({relationshipId, sequenceNumber: {$lt: user.sequenceNumber, $gt: relationship.contextAfterSequence || 0}, status: "completed"})
    ...
    const result = parseExtraction(await llm.generateJson({signal, messages: [
        ...
        {role: "user", content: JSON.stringify({previousRelationshipSummary: relationship.relationshipSummary, recentContext: recent.reverse().map(({role, content}) => ({role, content: content.slice(-2000)})), stage: relationship.stage, introduction: relationship.introduction,
            conversation: [{role: "user", content: user.content}, {role: "assistant", content: assistant.content}]})},
    ]}));
    const values = [];
    ...
    await mongoose.connection.transaction(async session => {
        ...
        for (const value of values) {
            ...
            const data = {relationshipId, userId: current.userId, characterId: current.characterId,
                ...
                sourceMessageIds: [userMessageId, assistantMessageId], sourceSequence: assistant.sequenceNumber,
            };
            if (previous) {
                if (previous.text === value.text) {
                    ...
                    previous.sourceMessageIds.addToSet(userMessageId, assistantMessageId);
        ...
        if ((current.summarySequence || 0) < assistant.sequenceNumber) {
            ...
            if (update?.stage && update.evidence && user.content.includes(update.evidence)) {
                ...
                if (allowed[current.stage]?.includes(update.stage)) {
