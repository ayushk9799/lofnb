...
const schema = z.object({
  ...
  GOOGLE_CLIENT_ID: optionalString.default(
    "50299044849-tl8kc7h49rcbl5aicfs41eg49tf3bmkn.apps.googleusercontent.com",
  ),
...
export function loadEnvironment(source = process.env) {
  const isOpenRouter = Boolean(
    ...
    (source.LLM_BASE_URL &&
      String(source.LLM_BASE_URL).includes("openrouter.ai")),
  );
  const effectiveApiKey =
    source.OPENROUTER_API_KEY || source.LLM_API_KEY || source.OPENAI_API_KEY;
  const effectiveEmbeddingKey =
    source.EMBEDDING_API_KEY ||
    source.OPENROUTER_API_KEY ||
    source.OPENAI_API_KEY ||
    source.LLM_API_KEY;
  const defaultBaseUrl = isOpenRouter
  ...
  const normalized = {
    ...
    MONGODB_URI:
      ...
      (source.NODE_ENV === "test"
        ? "mongodb://localhost:27017/lofn_test"
        : undefined),
    COMPANION_OFFLINE_MINUTES:
      source.COMPANION_OFFLINE_MINUTES || minutesFromHours,
    LLM_API_KEY: effectiveApiKey,
    LLM_BASE_URL:
      source.LLM_BASE_URL || (effectiveApiKey ? defaultBaseUrl : undefined),
    LLM_MODEL:
      source.LLM_MODEL ||
      source.OPENAI_MODEL ||
